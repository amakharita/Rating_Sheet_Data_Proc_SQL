...
SELECT rl.policynumber 'Policy_Number', 
	   ...
	   (SELECT CAST(MAX([BAPU_PolicyAdministration].global.CustomProperty.PropertyValue) AS numeric)
	    FROM bapu_policyadministration.PolicyAdministration.Policy
			INNER JOIN bapu_policyadministration.PolicyAdministration.PolicyRiskLocation 
			ON PolicyRiskLocation.PolicyId = Policy.PolicyId
			LEFT JOIN [BAPU_PolicyAdministration].Global.CustomProperty 
			ON CustomProperty.InstanceId = PolicyRiskLocation.PolicyRiskLocationId
			LEFT JOIN [BAPU_PolicyAdministration].Global.CustomPropertyDefinition 
	   ...
	   (SELECT MAX([BAPU_PolicyAdministration].global.CustomProperty.PropertyValue) AS CaliforniaEarthquakeLimitAmount
	    FROM bapu_policyadministration.PolicyAdministration.Policy
			INNER JOIN bapu_policyadministration.PolicyAdministration.PolicyRiskLocation 
			ON PolicyRiskLocation.PolicyId = Policy.PolicyId
			LEFT JOIN [BAPU_PolicyAdministration].Global.CustomProperty 
...
FROM  (SELECT p.policynumber, 
		...
		FROM bapu_policyadministration.PolicyAdministratiON.Policy p
			LEFT JOIN bapu_policyadministration.[PolicyAdministratiON].[PolicyRisklocation] prl  
			ON p.policyid = prl.policyid
			LEFT JOIN bapu_policyadministration.[OrganizatiON].[Address] addr 
			ON addr.Parentid = prl.PolicyRisklocationId 
			LEFT JOIN bapu_policyadministration.PolicyAdministration.PolicyRisklocationbuilding prlb 
	...
	LEFT JOIN (SELECT PolicyRisklocation.PolicyRisklocationId, 
				...
				FROM bapu_policyadministration.PolicyAdministratiON.Policy
					INNER JOIN bapu_policyadministration.PolicyAdministratiON.PolicyRisklocation 
					ON PolicyRisklocation.PolicyId = Policy.PolicyId
					INNER JOIN bapu_policyadministration.GLOBAL.CustomProperty 
					ON CustomProperty.InstanceId = PolicyRisklocation.PolicyRisklocationId
					INNER JOIN bapu_policyadministration.GLOBAL.CustomPropertyDefinitiON 
					ON CustomProperty.CustomPropertyDefinitiONId = CustomPropertyDefinitiON.CustomPropertyDefinitiONId
					INNER JOIN bapu.dbo.v_bus 
					ON v_bus.policynumber = policy.policynumber
					INNER JOIN (SELECT policy.[PolicyNumber],
										Policy.PolicyId,
										PolicyLimit.Amount
								FROM bapu_policyadministration.[PolicyAdministratiON].[Policy]
									INNER JOIN bapu_policyadministration.PolicyAdministratiON.PolicyCoveragePart 
									ON Policy.PolicyId = PolicyCoveragePart.PolicyId
									INNER JOIN bapu_policyadministration.PolicyAdministratiON.PolicyLimit 
									ON PolicyLimit.PolicyCoveragePartId = PolicyCoveragePart.PolicyCoveragePartId
									INNER JOIN bapu_policyadministration.ProductDefinitiON.LimitDefinition 
	...
	LEFT JOIN (SELECT policynumber, 
				...
				FROM bapu_policyadministration.[PolicyAdministratiON].[Policy]
					...
					INNER JOIN bapu_policyadministration.ProductDefinitiON.LimitDefinition 
					ON PolicyLimit.LimitDefinitionId = LimitDefinition.LimitDefinitionId
					INNER JOIN bapu_policyadministration.PolicyAdministratiON.PolicyDeductibleSIR  
	...
	LEFT JOIN (SELECT policynumber, 
				...
				FROM bapu_policyadministration.[PolicyAdministratiON].[Policy]
					...
					INNER JOIN bapu_policyadministration.ProductDefinitiON.LimitDefinition 
					ON PolicyLimit.LimitDefinitionId = LimitDefinition.LimitDefinitionId
					LEFT JOIN BAPU_PolicyAdministration.PolicyAdministration.PolicyInsured pins 
					ON policy.PolicyId = pins.PolicyId
					LEFT JOIN BAPU_PolicyAdministration.PolicyAdministration.PolicyNamedInsured nins 
					ON pins.PolicyInsuredId = nins.PolicyInsuredId
					LEFT JOIN BAPU_PolicyAdministration.Organization.Division div 
					ON policy.DivisionId = div.DivisionId
					LEFT JOIN BAPU_PolicyAdministration.Organization.UnderwritingEmployee ue 
	...
	LEFT JOIN (SELECT policynumber,
				...
				FROM (SELECT pol.policynumber,
						...
						FROM BAPU_PolicyAdministration.PolicyAdministration.PolicyCoveragePart pcp
							LEFT JOIN BAPU_PolicyAdministration.PolicyAdministration.PolicyLimit pl 
							ON pcp.PolicyCoveragePartId = pl.PolicyCoveragePartId
							LEFT JOIN BAPU_PolicyAdministration.PolicyAdministration.Policy pol 
